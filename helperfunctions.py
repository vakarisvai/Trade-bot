...
def trade(
    ...
    previous_rsi: float,
    short_term_ma: float,
    previous_short_term_ma: float,
    long_term_ma: float,
    previous_long_term_ma: float,
) -> int:
    """
    ...
    """
    if (previous_short_term_ma < previous_long_term_ma) & (
        short_term_ma > long_term_ma
    ):
        return 2
    elif (previous_short_term_ma > previous_long_term_ma) & (
        short_term_ma < long_term_ma
    ):
        return 1
...
def inform_subscribers(trade_result: int, ticker: str) -> None:
    ...
    """
    ssm_client = boto3.client("ssm")
    aws_access_key = ssm_client.get_parameter(
        Name="my_aws_access_key", WithDecryption=True
    )["Parameter"]["Value"]
    aws_secret_access_key = ssm_client.get_parameter(
        Name="my_aws_secret_access_key", WithDecryption=True
    )["Parameter"]["Value"]
    gmail_password = ssm_client.get_parameter(Name="tc_gmail_key", WithDecryption=True)[
        "Parameter"
    ]["Value"]
    ...
    if trade_result == 1:
        message_body = (
            f"According to RSI and moving average indicators {ticker} stock is a buy"
        )
    else:
        message_body = (
            f"According to RSI and moving average indicators {ticker} stock is a sell"
        )
